...
from hearts_prediction.core import metrics, models, schemas
from hearts_prediction.io import datasets, registries, services
from hearts_prediction.utils import searchers, signers, splitters
...
def outputs_path(data_path: str) -> str:
    ...
    return os.path.join(data_path, "outputs_sample.csv")
...
def outputs_reader(
    ...
    if not os.path.exists(outputs_path):
        ...
        model = models.BaselineSklearnXGBModel().fit(inputs=inputs, targets=targets)
        outputs = schemas.OutputsSchema.check(model.predict(inputs=inputs))
        outputs_writer = datasets.CSVWriter(path=outputs_path)
        outputs_writer.write(data=outputs)
    return datasets.CSVReader(path=outputs_path, limit=LIMIT)
...
@pytest.fixture(scope="session")
def searcher() -> searchers.BayesCVSearcher:
    ...
    return searchers.BayesCVSearcher(param_grid=param_grid)
...
) -> models.BaselineSklearnXGBModel:
    """Return a train model for testing."""
    model = models.BaselineSklearnXGBModel()
