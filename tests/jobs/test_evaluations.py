...
from hearts_prediction.core import metrics, schemas
...
@pytest.mark.parametrize(
    ...
    [
        (
            1,
            {"f1_score": metrics.Threshold(threshold=0.001, greater_is_better=True)},
        ),
        (
            "Promotion",
            {"f1_score": metrics.Threshold(threshold=-1, greater_is_better=True)},
        ),
        pytest.param(
            "Promotion",
            {"f1_score": metrics.Threshold(threshold=0.001, greater_is_better=True)},
...
def test_evaluations_job(
    ...
    assert (
        out["dataset"].targets == schemas.TargetsSchema.HeartDisease
